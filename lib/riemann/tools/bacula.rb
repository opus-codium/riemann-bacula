      ...
      def process_stdin
        ...
        while (line = $stdin.gets)
          ...
          value = case raw_value
                  ...
                  when /\A(?:(\d+ mins ))?(\d+) secs\z/ then (Regexp.last_match(2).to_i || 0) * 60 + Regexp.last_match(1).to_i
          ...
          if key == 'FileSet'
            value =~ /\A"([^"]+)" ([^ ]+)/
            data['FileSet Date'] = Regexp.last_match(2)
            value = Regexp.last_match(1)
          end

          data[key] = value

          next unless key == 'Termination'
          ...
          [
            ...
            event = {}
            event[:service] = "backup #{data['Job Name']} #{event_name.downcase}"
          ...
          event[:level] = data['Backup Level']
          # event[:time] = Time.parse(data['Start time']).to_i
